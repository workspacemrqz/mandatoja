/**
 * Sistema de fila global para gerenciar requisições ao Ollama API
...
export class OllamaQueue {
  private static instance: OllamaQueue;
  ...
  /**
   * Obtém a instância única do OllamaQueue (Singleton)
   */
  public static getInstance(): OllamaQueue {
    if (!OllamaQueue.instance) {
      OllamaQueue.instance = new OllamaQueue();
    }
    return OllamaQueue.instance;
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        console.error("[OllamaQueue] ❌ Erro ao processar requisição:", error);
        item.reject(error);
      }
...
- Mantenha respostas curtas e naturais (1-3 frases no máximo)`;
      ...
      // Parsear contexto de eleitor (usado no Clone Agent)
      const contextParts = messageContext.split(/===\s*(HISTÓRICO DE CONVERSAS|MENSAGEM ATUAL)\s*===/);
      const voterInfo = contextParts[0]?.trim();
      
      if (voterInfo && voterInfo.length > 0 && voterInfo.includes('Eleitor:')) {
        messages.push({
          role: "system",
          content: `CONTEXTO DO ELEITOR:\n${voterInfo}`
        });
      }

      // Parsear mensagens de conversação
      const conversationLines = messageContext.split('\n');
      for (const line of conversationLines) {
        if (line.startsWith('Eleitor:') || line.startsWith('Usuário:')) {
          const content = line.replace(/^(Eleitor|Usuário):/, '').trim();
          if (content) {
            messages.push({
              role: "user",
              content: content
          ...
          const content = line.replace('Assistente:', '').trim();
          if (content) {
            messages.push({
              role: "assistant",
              content: content
          ...
          // Mensagens do militant agent vem no formato "Nome: mensagem"
          // Extrair apenas a mensagem, ignorando o nome
          ...
          if (colonIndex > 0 && colonIndex < 50) {
            ...
            // Se não tem ":", enviar a linha inteira
            messages.push({
              role: "user",
              content: line.trim()
            });
      ...
      if (!response.ok) {
        ...
        console.error("[OllamaQueue] Erro na API:", response.status, errorText);
      ...
      if (data.message) {
        ...
        if (!generatedResponse) {
          console.error("[OllamaQueue] Resposta da API vazia");
          return null;
        }
        
        console.log(`[OllamaQueue] 📝 Resposta bruta (primeiros 500 chars): ${generatedResponse.substring(0, 500)}`);
        
        generatedResponse = this.cleanResponse(generatedResponse);
        
        console.log(`[OllamaQueue] ✅ Resposta limpa (primeiros 300 chars): ${generatedResponse.substring(0, 300)}`);
        ...
        if (!generatedResponse || generatedResponse.length < 3) {
          console.error("[OllamaQueue] Resposta vazia após limpeza");
          return null;
        }
        
        if (generatedResponse.length > 300) {
          console.log(`[OllamaQueue] ⚠️ Resposta muito longa (${generatedResponse.length} chars), truncando para 300 chars`);
          const lastPunctuation = generatedResponse.substring(0, 300).lastIndexOf('.');
          if (lastPunctuation > 100) {
            generatedResponse = generatedResponse.substring(0, lastPunctuation + 1);
          } else {
            generatedResponse = generatedResponse.substring(0, 300) + '...';
          }
        }
      ...
      console.error("[OllamaQueue] Resposta da API não contém message");
      return null;

    } catch (error: any) {
      console.error("[OllamaQueue] Erro ao chamar Ollama API:", error.message);
      throw error;
    }
  }
  ...
  private cleanResponse(text: string): string {
    ...
    if (hasLeak) {
      console.warn('[OllamaQueue] ⚠️ VAZAMENTO DETECTADO após limpeza');
      console.warn(`[OllamaQueue] Texto problemático: ${cleaned.substring(0, 200)}`);
...
export const ollamaQueue = OllamaQueue.getInstance();
